...
if len(sys.argv) < 2:
   print("Usage: ravenna_hkspi.py <file>")
   sys.exit()
...
# Do not attempt any communication without first putting the
# Ravenna processor itself into reset.
...
with open(file_path, mode='r') as f:
    ...
    while x != '':
        ...
        if x[0] == '@':
            if nbytes > 0:
                nextaddr = int(x[1:],16)
                if nextaddr != addr + nbytes:
                    doflush = True
                    print('setting address to {}'.format(hex(nextaddr)))
                    print('addr = ' + str(addr) + ' nbytes = ' + str(nbytes))
            else:
                addr = int(x[1:],16)
